...
async def show_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    menu_text = [f"🎵 *Groove AI Radio v2.1-final* 🎵", "", f"💿 *Commands*:", "`/play, /p <query>` - Найти и проиграть трек", "`/menu, /m` - Показать это меню"]
    reply_keyboard_markup = ReplyKeyboardRemove()
    if is_admin_user:
        menu_text.extend(["", f"👑 *Admin Commands*:", "`/ron, /r_on` - Включить радио", "`/roff, /r_off, /stop, /t` - Выключить радио", "`/skip, /s` - Пропустить трек", "`/vote, /v` - Голосование за жанр", "`/source, /src <source>` - Сменить источник (yt, sc, vk, ar)", "`/keyboard` - Показать/скрыть клавиатуру", "`/stopbot` - Остановить бота"])
        reply_keyboard_markup = ReplyKeyboardMarkup([['/ron', '/roff', '/skip'], ['/src yt', '/src sc', '/src vk', '/src ar'], ['/vote', '/refresh']], resize_keyboard=True, input_field_placeholder="Admin Commands")
