...
# ... (All other bot command functions like play_command, button_callback, etc. remain exactly the same) ...
...
# --- Music Handling (No changes needed here) ---
...
# --- Main Application Logic (Webhook with Health Check) ---
async def main() -> None:
    if not BOT_TOKEN or not WEBHOOK_URL:
        logger.critical("FATAL: BOT_TOKEN and WEBHOOK_URL must be set.")
        return
    ...
    # Define the web server handlers
    ...
    webapp.add_routes([
        web.get("/", health_check), # Health check endpoint
        web.post(f"/{BOT_TOKEN}", telegram_webhook), # Telegram updates endpoint
    ])
    ...
    logger.info(f"Starting custom web server on port {port}...")
    await site.start()
    ...
    await runner.cleanup() # aiohttp cleanup

if __name__ == "__main__":
    ensure_download_dir()
    logger.info("Starting application in custom webhook mode...")
    try:
        asyncio.run(main())
