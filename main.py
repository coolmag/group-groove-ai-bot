...
from config import (
    BOT_TOKEN, BotState, ChatData, MESSAGES, check_environment,
...
class MusicBot:
    def __init__(self, app: Application):
        ...
        # Hourly vote
        self.job_queue.run_repeating(self.start_hourly_vote, interval=3600, first=self._seconds_to_next_hour())
    ...
    async def on_button(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data.startswith("pick:"):
            ...
            if not results or idx < 0 or idx >= len(results):
                ...
                return
            sel = results[idx]
            # очистим результаты после выбора, чтобы не копить
            async with state_lock:
    ...
    async def update_status_message(self, context: ContextTypes.DEFAULT_TYPE):
        ...
        for cid, chat_data in list(self.state.active_chats.items()):
            try:
                ...
                async with state_lock:
                    self.state.active_chats.pop(cid, None)
            except Exception as e:
    ...
    async def _update_status_for_chat(self, context: ContextTypes.DEFAULT_TYPE, chat_id: int):
        ...
        try:
            if chat_data and chat_data.status_message_id:
                ...
                async with state_lock:
                    self.state.active_chats[chat_id].status_message_id = sent.message_id
        except Exception as e:
